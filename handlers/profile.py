...
from keyboards import back_to_menu_kb, payment_kb, profile_kb, topup_wallets_kb, topup_methods_kb
from models import User
from services.balance_service import BalanceService
from services.order_service import OrderService
from services.user_service import UserService
from states import TopUpStates, AdminVIPSettingsStates
...
async def cb_vip_buy(callback: CallbackQuery, db_user: User, state: FSMContext):
    ...
    await state.update_data(is_vip_purchase=True)
...
async def on_vip_amount(message: Message, state: FSMContext, db_user: User):
    """Пользователь ввёл сумму для VIP — проверить и создать инвойс."""
    amount = validate_price(message.text)
    if amount is None:
        await message.answer("❌ Введите корректную сумму (число больше 0):")
        return
    ...
    # Show topup methods (manual + crypto)
    from keyboards import topup_methods_kb
    await message.answer(
        f"💳 <b>Способ оплаты</b>\n\n"
        f"Сумма: {format_price(amount)}\n\n"
        "Выберите способ:",
...
async def cb_topup_method(callback: CallbackQuery, state: FSMContext, db_user: User):
    ...
    if method == "crypto":
        ...
        try:
            ...
            # Save crypto invoice for tracking
            from database import get_db
            async with get_db() as db:
                await db.execute(
                ...
                await db.execute(
                    """INSERT INTO balance_topups (user_id, amount, status, receipt_type)
                       VALUES (?, ?, 'pending', 'crypto')""",
                    (db_user.id, amount),
                )
